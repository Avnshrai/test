...
if [[ "$1" = "/opt/coredge/scripts/grafana/run.sh" ]] || ! is_exec "$1"; then
    ...
    /opt/coredge/scripts/grafana/setup.sh || GRAFANA_OPERATOR_IMMEDIATE_EXIT=$?
...
if is_exec "$1"; then
    ...
    exec "/opt/coredge/scripts/grafana/run.sh" "$@"
fi