...
web_server_execute() {
    ...
    (
        . "/opt/coredge/scripts/lib${web_server}.sh"
        . "/opt/coredge/scripts/${web_server}-env.sh"
        "$@"
    )
}
...
web_server_list() {
    ...
    for web_server in "${supported_web_servers[@]}"; do
        [[ -f "/opt/coredge/scripts/${web_server}-env.sh" ]] && existing_web_servers+=("$web_server")
    done
...
web_server_validate() {
    ...
    if [[ -z "$(web_server_type)" || ! " ${supported_web_servers[*]} " == *" $(web_server_type) "* ]]; then
        ...
        print_validation_error "Could not load the $(web_server_type) web server library from /opt/coredge/scripts. Check that it exists and is readable."
    fi
...
web_server_start() {
    ...
    if [[ "${COREDGE_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl start "coredge.$(web_server_type).service"
    else
        "${COREDGE_ROOT_DIR}/scripts/$(web_server_type)/start.sh"
    fi
}
...
web_server_stop() {
    ...
    if [[ "${COREDGE_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl stop "coredge.$(web_server_type).service"
    else
        "${COREDGE_ROOT_DIR}/scripts/$(web_server_type)/stop.sh"
    fi
}
...
web_server_restart() {
    ...
    if [[ "${COREDGE_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl restart "coredge.$(web_server_type).service"
    else
        "${COREDGE_ROOT_DIR}/scripts/$(web_server_type)/restart.sh"
    fi
}
...
web_server_reload() {
    if [[ "${COREDGE_SERVICE_MANAGER:-}" = "systemd" ]]; then
        systemctl reload "coredge.$(web_server_type).service"
    else
        "${COREDGE_ROOT_DIR}/scripts/$(web_server_type)/reload.sh"
    fi
}
