...
This chart bootstraps a [Jenkins] deployment on a [Kubernetes](https://kubernetes.io) cluster using the [Helm](https://helm.sh) package manager.
...
Step 1: Navigate to the root directory of your Helm chart: Open a terminal or command prompt and navigate to the root directory of your Helm chart. This is where the Chart.yaml file are located and now just run the below command to install dependencies -
...
| `image.repository`  | Jenkins image repository                                                                                | `coredge/jenkins`      |
...
| `jenkinsHome`            | Jenkins home directory                                                                                                                              | `/coredge/jenkins/home` |
...
| `skipImagePlugins`       | Set this value to true to skip installing plugins stored under /opt/coredge/jenkins/plugins                                                         | `false`                 |
...
| `agent.image.repository`                                                  | Jenkins image repository                                                                                | `coredge/jenkins-agent`  |
...
| `volumePermissions.image.tag`                 | Coredge Shell image tag (immutable tags are recommended)                                                      | `11-debian-11-r118`     |
...
```console
helm install my-release -f values.yaml <path-to-chart>
```
...
The [Coredge Jenkins] image stores the Jenkins data and configurations at the `/coredge/jenkins` path of the container. Persistent Volume Claims (PVCs) are used to keep the data across deployments.

## License

Copyright &copy; 2023 Coredge, Inc.
