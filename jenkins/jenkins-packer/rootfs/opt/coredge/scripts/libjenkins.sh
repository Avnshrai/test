#!/bin/bash
#
# Coredge Jenkins library

# shellcheck disable=SC1091
# shellcheck disable=SC1090

JAVA_HOME=/opt/coredge/java

# Load generic libraries
. /opt/coredge/scripts/libfile.sh
. /opt/coredge/scripts/libfs.sh
. /opt/coredge/scripts/libnet.sh
. /opt/coredge/scripts/libos.sh
. /opt/coredge/scripts/libservice.sh
. /opt/coredge/scripts/libvalidations.sh
. /opt/coredge/scripts/libpersistence.sh
...
jenkins_initialize() {
    ...
    if is_mounted_dir_empty "$JENKINS_HOME"; then
        ...
        if ! is_mounted_dir_empty "$JENKINS_MOUNTED_CONTENT_DIR"; then
            ...
            find "$JENKINS_MOUNTED_CONTENT_DIR" \( -type f -o -type l \) -and -not -path "$JENKINS_MOUNTED_CONTENT_DIR/plugins/*" | xargs -I % -P10 bash -c '. /opt/coredge/scripts/libjenkins.sh && jenkins_add_custom_file %'
        fi
        ...
        if ! is_boolean_yes "$JENKINS_SKIP_BOOTSTRAP"; then
            ...
            info "Skipping Coredge initialization"
        fi
    else
...
jenkins_install_plugins() {
    ...
    if ! is_mounted_dir_empty "$JENKINS_MOUNTED_CONTENT_DIR/plugins"; then
        ...
        echo "--- Copying files at $(date)" >>"${JENKINS_LOGS_DIR}/copy_reference_file.log"
        find "$JENKINS_MOUNTED_CONTENT_DIR/plugins" \( -type f -o -type l \) | xargs -I % -P10 bash -c '. /opt/coredge/scripts/libjenkins.sh && jenkins_add_custom_file %'
    else
...
jenkins_override_home_paths() {
    ...
    for path in "${paths_list[@]}"; do
        ...
        if ! is_mounted_dir_empty "$JENKINS_MOUNTED_CONTENT_DIR/${relpath}"; then
            ...
            find "$JENKINS_MOUNTED_CONTENT_DIR/${relpath}" \( -type f -o -type l \) | xargs -I % -P10 bash -c '. /opt/coredge/scripts/libjenkins.sh && jenkins_add_custom_file %'
