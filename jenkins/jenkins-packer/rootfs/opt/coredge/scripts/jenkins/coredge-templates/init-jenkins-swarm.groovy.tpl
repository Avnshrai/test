...
println " [coredge/groovy-init-jenkins-with-slaves] Creating Jenkins users"
...
println " [coredge/groovy-init-jenkins-with-slaves] Admin user created: {{JENKINS_USERNAME}}:*******"
...
println " [coredge/groovy-init-jenkins-with-slaves] Slave user created: {{JENKINS_SWARM_USERNAME}}:*******"
...
println " [coredge/groovy-init-jenkins-with-slaves] System user created: system_user:*******"

// Set Authorization strategy
// Roles based on https://wiki.jenkins-ci.org/display/JENKINS/Matrix-based+security
println " [coredge/groovy-init-jenkins-with-slaves] Setting Authorization Strategy"
...
println " [coredge/groovy-init-jenkins-with-slaves] Authorization Strategy set"
...
println " [coredge/groovy-init-jenkins-with-slaves] Configuring 'Authorize Project' plugin"
...
for (authenticator in authenticators) {
  if (authenticator instanceof GlobalQueueItemAuthenticator) {
    println " [coredge/groovy-init-jenkins-with-slaves]     Skipping global build authenticator, it exists"
    configureGlobalAuthenticator = false
  } else if (authenticator instanceof ProjectQueueItemAuthenticator) {
    println " [coredge/groovy-init-jenkins-with-slaves]     Skipping per-project build authenticator, it exists"
...
if (configureGlobalAuthenticator) {
  ...
  println " [coredge/groovy-init-jenkins-with-slaves]     Configuring global build authenticator with '${globalStrategyName}' strategy"
...
if (configureProjectAuthenticator) {
  ...
  println " [coredge/groovy-init-jenkins-with-slaves]     Configuring per-project build authenticator"
  println " [coredge/groovy-init-jenkins-with-slaves]         Allowing '${anonymousAuthorizationStrategyDescriptor.getDisplayName()}' strategy"
  println " [coredge/groovy-init-jenkins-with-slaves]         Allowing '${triggeringUsersAuthorizationStrategyDescriptor.getDisplayName()}' strategy"
  println " [coredge/groovy-init-jenkins-with-slaves]         Allowing '${specificUsersAuthorizationStrategyDescriptor.getDisplayName()}' strategy"
  authenticators.add(new ProjectQueueItemAuthenticator(projectStrategy))
}
println " [coredge/groovy-init-jenkins-with-slaves] 'Authorize Project' plugin configuration finished"

// Configure JNLP port
println " [coredge/groovy-init-jenkins-with-slaves] Configuring JNLP port"
jenkins.setSlaveAgentPort({{jnlp_port}})
println " [coredge/groovy-init-jenkins-with-slaves] JNLP port is set to '{{jnlp_port}}'"
...
println " [coredge/groovy-init-jenkins] master-slave set"

// Set master executors
println " [coredge/groovy-init-jenkins] Setting master executors to 0"
jenkins.setNumExecutors(0);
println " [coredge/groovy-init-jenkins] master executors set"

jenkins.save()

// Complete wizard
println " [coredge/groovy-init-jenkins-with-slaves] Passing wizard"
def wizard = new SetupWizard()
wizard.init(true)
wizard.completeSetup()
println " [coredge/groovy-init-jenkins-with-slaves] Wizard passed"
