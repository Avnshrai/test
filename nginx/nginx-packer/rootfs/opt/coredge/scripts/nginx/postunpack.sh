...
rm -rf "${COREDGE_ROOT_DIR}/certs" "${COREDGE_ROOT_DIR}/server_blocks"

# Ensure non-root user has write permissions on a set of directories
for dir in "$NGINX_VOLUME_DIR" "$NGINX_CONF_DIR" "$NGINX_INITSCRIPTS_DIR" "$NGINX_SERVER_BLOCKS_DIR" "${NGINX_CONF_DIR}/coredge" "${NGINX_CONF_DIR}/coredge/certs" "$NGINX_LOGS_DIR" "$NGINX_TMP_DIR"; do
...
nginx_configure_port "$NGINX_DEFAULT_HTTPS_PORT_NUMBER" "${COREDGE_ROOT_DIR}/scripts/nginx/coredge-templates/default-https-server-block.conf"
...
mv "${NGINX_CONF_DIR}/coredge/certs" /certs
ln -sf /certs "${NGINX_CONF_DIR}/coredge/certs"
