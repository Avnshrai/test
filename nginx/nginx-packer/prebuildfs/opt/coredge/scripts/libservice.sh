...
generate_systemd_conf() {
    ...
    local -r service_file="${systemd_units_dir}/coredge.${service_name}.service"
    # Default values
...
[Install]
# Enabling/disabling the main coredge service should cause the same effect for this service
WantedBy=coredge.service
EOF
}
